			...
			<div class="rounded-xl bg-white border border-slate-200 shadow-sm p-5 animate-result-reveal">
						...
						<div class="flex flex-col gap-2">
							{#each diagnosis.differentials as diff}
								<div class="flex items-center gap-3">
									<span class="text-xs text-slate-600 w-40 shrink-0 truncate">{diff.diagnosis}</span>
									<div class="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
										<div class="h-full bg-slate-400 rounded-full transition-[width] duration-700 ease-out"
											style="width: {(diff.confidence * 100).toFixed(0)}%"></div>
									</div>
									<span class="text-xs font-mono text-slate-500 shrink-0 w-10 text-right">{(diff.confidence * 100).toFixed(0)}%</span>
								</div>
							{/each}
						</div>
					</div>
				...
				{#if diagnosis.reasoning}
					<div class="rounded-lg bg-slate-50 border border-slate-100 p-4 mb-4">
						<p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">{m.diagnosis_reasoning()}</p>
						<p class="text-sm text-slate-700 leading-relaxed">{diagnosis.reasoning}</p>
					</div>
				{/if}
